...
export function Men() {
  ...
  const totalUnitsLeft = menWatches.reduce((total, watch) => total + watch.stockLeft, 0);
    ...
    <main className="bg-[whitesmoke] py-12">
      <section className="mx-auto max-w-[1400px] px-4 md:px-8">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            <p className="text-sm tracking-[0.2em] text-zinc-500">MEN COLLECTION</p>
            <h1 className="font-heading text-3xl md:text-5xl text-zinc-900 mt-2">Watches For Men</h1>
          </div>
          <div className="bg-white border border-zinc-200 rounded-lg px-4 py-3 text-sm md:text-base text-zinc-700">
            Products: <span className="font-semibold">{menWatches.length}</span> | Units left:{" "}
